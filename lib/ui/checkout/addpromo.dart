...
class _AddPromoScreenState extends State<AddPromoScreen> {
  ...
  void initState() {
    ...
    coupons = getVisibleCoupons(true, _mainModel.currentService);
    super.initState();
  }
  ...
  List<Widget> _child(){
    ...
    list.add(
      Row(
        children: [
          Expanded(child: Text(strings.get(282),  /// "My rewards",
          ...
          InkWell(
            ...
            child: Row(children: [
            Text(strings.get(280),  /// "Enter code",
              style: theme.style14W800,),
            SizedBox(width: 5,),
    ...
    for (var item in coupons){
      ...
      list.add(InkWell(
        onTap: (){
          ...
          child: offerWidget(item.color, strings.get(283), /// "Special offer"
          theme.style12W600White, item.image,
          disc, theme.style14W800,
          windowWidth-30, windowWidth*0.4,
          strings.get(284), theme.style14W600White, /// "Use now"
          "${strings.get(285)} ${appSettings.getDateTimeString(item.expired)}", theme.style12W800)  /// "Valid until",
      )));
      if (item.state.isNotEmpty){
        list.add(SizedBox(height: 10,));
        var _text = "";
        if (item.state == "not_found")
          _text = strings.get(188); /// "Coupon not found",
        if (item.state == "expired")
          _text = strings.get(189); /// "Coupon has expired",
        if (item.state == "not_supported_by_this_provider")
          _text = strings.get(190); /// "Coupon not supported by this provider",
        if (item.state == "not_support_this_category")
          _text = strings.get(191); /// "Coupon not support this category",
        if (item.state == "not_support_this_service")
          _text = strings.get(192); /// "Coupon not support this category",
        if (item.state == "not_support_this_product")
          _text = strings.get(281); /// "Coupon not support this product",
