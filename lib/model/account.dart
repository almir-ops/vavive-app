...
class MainModelUserAccount{
        ...
        FirebaseFirestore.instance.collection("listusers").doc(user.uid).get().then((querySnapshot) async {
          if (querySnapshot.exists){
            ...
            if (providers.isEmpty) {
              loadProvider(() async {
                var ret = await initChatV2((){});
                if (ret != null)
                  messageError(context, ret);
              });
            }else{
              ret = await initChatV2((){});
              if (ret != null)
                messageError(context, ret);
            }
            initCart();
  ...
  //
   ...
   Future<String?> saveLocation(int _type, String _address, String _name, String _phone) async {
    if (_address.isEmpty)
      return strings.get(65); /// "Please enter address",
    if (_name.isEmpty)
      return strings.get(123); /// "Please Enter name",
    if (_phone.isEmpty)
      return strings.get(176); /// "Please enter phone",
