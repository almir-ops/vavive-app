...
export class ProfileComponent implements OnInit {
  ...
  getInfoUser() {
    this.getUserSecurely()
      .then((user) => {
        if (user) {
          console.log('Usuário recuperado:', user);
          this.user = user.cliente;
          console.log(this.user);
          this.setFormValues(user.cliente);
        } else {
  ...
  updateClient() {
    ...
    this.clienteService.updateClient(cliente, this.user.source_api).subscribe({
      next: (value: any) => {
        console.log(value);
        Preferences.set({
          key: 'user_data',
          value: JSON.stringify({ cliente: value.item }),
        });
      },
      error: (err: any) => {
        console.log(err);
  ...
  openModalDeleteEndereco(endereco: any, index: any) {
    if (index > 0) {
      ...
      this.alertService.presentAlert(
        'Atenção ',
        `Adicione um novo endereço para excluir o principal.`
      );
    }
  }
        ...
        next: (endereco) => {
          if (!endereco.erro) {
            ...
            if (endereco.uf === 'DF') {
              ...
              franquia = this.regions.find(
                (r: any) => r.estado.toLowerCase() === 'df'
              );
            } else {
              ...
              franquia = this.regions.find((r: any) =>
                r.bairros.some(
                  (bairroObj: any) =>
                    (typeof bairroObj === 'string'
                      ? bairroObj.toLowerCase()
                      : bairroObj.nome.toLowerCase()) ===
                    endereco.localidade.toLowerCase()
                )
              );
            }
            ...
            if (!franquia && (endereco.uf === 'SP' || endereco.uf === 'RJ')) {
              franquia = this.regions.find(
                (r: any) => r.nome.toLowerCase() === 'matriz'
              );
            }
        ...
        async (endereco: any) => {
          if (endereco && !endereco.erro) {
            ...
            if (region) {
              ...
              console.log(
                'Cidade não encontrada. Comparando com:',
                endereco.localidade
              );
            }
            ...
            if (!region && endereco.uf === 'DF') {
              ...
              if (region) {
                ...
                console.log(
                  'Estado DF não encontrado. Comparando com:',
                  endereco.uf
                );
              }
            }
            ...
            if (!region) {
              ...
              region = this.regions.find((r: any) => {
                ...
                console.log(
                  `Comparando CEP ${cepNumber} com faixa: ${startCep} - ${endCep}`
                );
              ...
              if (region) {
                ...
                console.log(
                  'CEP não encontrado na faixa. Comparando:',
                  cepNumber
                );
              }
            }
            ...
            if (!region) {
              ...
              region = this.regions.find(
                (r: any) =>
                  cepNumber >= parseInt(r.latitude, 10) &&
                  cepNumber <= parseInt(r.longitude, 10)
              );
            ...
            if (!region && endereco.uf === 'SP') {
              ...
              this.regionsCurrent = this.regions.reduce(
                (unique: any[], region: any) => {
  ...
  async preencherManual() {
    ...
    await this.storage.set('source', this.franquiaEncontrada.schema);

    console.log(this.endereco);

    console.log(this.franquiaEncontrada);
  ...
  selectFranquia(region: any) {
    ...
    this.storage.set('source', region.schema);
