...
export class SelectRegionComponent {
  ...
  @ViewChild('modalSejaFranqueado', { static: false })
  modalSejaFranqueado!: IonModal;
      ...
      this.viacepService.buscarEndereco(cep).subscribe(
        async (endereco: any) => {
          if (endereco && !endereco.erro) {
            ...
            const cidadeComFranquiaEncontrada = this.regions.some(
              (r: any) =>
                r.embed_cidade.toLowerCase() ===
            ...
            if (!region && cidadeComFranquiaEncontrada) {
              ...
              const franquiasDaCidade = this.regions.filter(
                (r: any) =>
                  r.embed_cidade.toLowerCase() ===
                  endereco.localidade.toLowerCase()
              );
              ...
              this.regionsCurrent = franquiasDaCidade.reduce(
            ...
            if (region) {
              ...
              await this.storage.set('schema', region.schema);
              await this.storage.set('current_cep', cep);
              console.log('Franquia encontrada:', region);
