...
export class ListServicesComponent implements OnInit {
  ...
  @ViewChild('modalDetailsProfissional', { static: false })
  modalDetailsProfissional!: IonModal;
  @ViewChild('modalEvaluation', { static: false }) modalEvaluation!: IonModal;
  @ViewChild('modalStatusCliente', { static: false })
  modalStatusCliente!: IonModal;
  ...
  getAtendimentos() {
    ...
    this.atendimentoService
      .filterAtendimentos('cliente_cpf', this.currentClient.cliente.cpf)
      .subscribe({
        next: (res: any) => {
          ...
          res.items.forEach((atendimento: any) => {
            ...
            if (
              ...
              atendimento.status_atendimento === 'Concluido'
            ) {
          ...
          this.listAgendados.sort((a: any, b: any) =>
            moment(a.data_inicio).diff(moment(b.data_inicio))
          );
          this.listHistorico.sort((a: any, b: any) =>
            moment(a.data_inicio).diff(moment(b.data_inicio))
          );
  ...
  isConfirmarAtendimentoDisponivel(): boolean {
    ...
    const dataInicioAtendimento = moment(
      this.currentAtendimento.data_inicio,
      'YYYY-MM-DD'
    );
    const horaDeEntradaAtendimento = moment(
      this.currentAtendimento.hora_de_entrada,
      'HH:mm'
    );

    const dataValida = dataInicioAtendimento.isSameOrBefore(dataAtual, 'day');
    const horaValida = horaDeEntradaAtendimento.isSameOrBefore(
      dataAtual,
      'minute'
    );
  ...
  async criaPagamento() {
    if (!this.currentAtendimento.transaction_receipt_url) {
      this.financasService
        .getFinancasByFilter('?atendimento_id=' + this.currentAtendimento.ID)
        .subscribe({
          next: (response: any) => {
            ...
            if (entradas.length === 0) {
              this.alertService.presentAlert(
                'Erro',
                'Nenhuma entrada encontrada para este atendimento.'
              );
              return;
            }
      ...
      await Browser.open({
        url: this.currentAtendimento.transaction_receipt_url,
      });
    }
  }
  ...
  openModalDetailsProfissional() {
    if (this.currentAtendimento.profissional.length > 0) {
      ...
      this.alertService.presentAlert(
        'Atenção ',
        `Atendimento sem profissional escalado.`
      );
    }
  }
  ...
  confirmedAtendimento() {
    if (this.currentAtendimento.profissional.length === 0) {
      this.alertService.presentAlert(
        'Atenção ',
        `Atendimento sem profissional, impossivel confirmar.`
      );
      return;
    } else {
  ...
  confirmCancel() {
    ...
    if (this.cancelDescription.length >= 5) {
      ...
      console.log(
        'Atendimento cancelado com o motivo:',
  ...
  updateDatasSelecionadas(dataAntiga: string, novaData: string) {
    ...
    if (typeof this.currentAtendimento.datas_selecionadas === 'string') {
      ...
      datasSelecionadasArray = JSON.parse(
        this.currentAtendimento.datas_selecionadas
      );
    } else {
    ...
    const updatedDatasSelecionadasArray = datasSelecionadasArray.map(
      (data: string) => {
    ...
    this.currentAtendimento.datas_selecionadas = JSON.stringify(
      updatedDatasSelecionadasArray
    );
  }

  onDateSelected(event: any) {
    this.updateDatasSelecionadas(
      this.currentAtendimento.data_inicio,
      event.detail.value
    );
  ...
  submitReview() {
    ...
    const reviewText = (
      document.getElementById('evaluation-textarea') as HTMLTextAreaElement
    ).value;
  ...
  criaRepasse(atendimento: any) {
    ...
    const getRepasseValue = async () => {
      let valorRepasse: number | undefined = await this.buscaValorRepasse(
        atendimento.duracao,
        atendimento.profissional[0].grupo_repasse
      );
      ...
      if (
        valorRepasse === 0 ||
        valorRepasse === undefined ||
        valorRepasse === null
      ) {
        valorRepasse = this.profissionalService.calcularValorRepasses(
          atendimento.duracao,
          atendimento.profissional[0].grupo_repasse
        );
    ...
    const processarRepasse = async () => {
      try {
        ...
        this.financasService
          .getRPagamentosByDatesAndFilter(
            atendimento.data_inicio,
            atendimento.data_inicio,
            '&tipo=Repasse&atendimento_id=' + atendimento.ID
          )
          .subscribe({
            next: (res: any) => {
  ...
  buscaValorRepasse(
    ...
    return new Promise((resolve, reject) => {
      this.repasseService
        .getRepassesByFilters(
          '?duracao=' + duracao + '&grupo_repasse=' + grupo_repasse
        )
        .subscribe({
          next: (res: any) => {
            if (
              res.items &&
              res.items.length > 0 &&
              res.items[0].valor != null
            ) {
              console.log('Valor encontrado ', res.items[0].valor);
              resolve(res.items[0].valor);
            } else {
              console.log(
                'Nenhum valor encontrado. Usando valor padrão ou undefined.'
              );
  ...
  updateProfissional(profissional: any) {
    this.profissionalService.putProfissional(profissional).subscribe(
      (response) => {
        console.log(response);
      },
      (error) => {
